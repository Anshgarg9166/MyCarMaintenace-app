...
class VehicleRegistrationFragment : Fragment() {
    ...
    private fun addNewVehicle() {
        if (isEntryValid(binding.vehicleType) &&
            ...
            val imageUriString = selectedImageUri?.toString() ?: "default_image_uri"
            
            try {
                vehiclesViewModel.addNewVehicle(
                    vehicleImageUri = imageUriString,
                ...
                Toast.makeText(this.context, "Error saving vehicle: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun bind(vehicle: Vehicle) {
        ...
        try {
            ...
            val defaultDrawable = changeVehicleTypeImage(requireContext(), vehicle.type, true)
            Picasso.get()
            ...
            // If there's an error loading the image, use the default vehicle type image
            val defaultDrawable = changeVehicleTypeImage(requireContext(), vehicle.type, true)
            binding.addVehicleImage.setImageDrawable(defaultDrawable)
        }
        ...
        if (isEntryValid(binding.vehicleType) &&
            ...
            try {
                val result = vehiclesViewModel.updateVehicle(
                    ...
                    vehicleImageUri = vehicle.vehicleImageUri,
                    vehicleType = binding.vehicleTypeEditText.text.toString(),
                    vehicleManufacturer = binding.vehicleManufacturerEditText.text.toString(),
                    vehicleModel = binding.vehicleModelEditText.text.toString(),
                    vehicleModelYear = binding.vehicleModelYearEditText.text.toString(),
                ...
                if (result) {
                    ...
                    findNavController().navigate(
                        VehicleRegistrationFragmentDirections
                            .actionVehicleRegistrationFragmentToVehicleDetailsFragment(this.navigationArgs.vehicleId)
                    )
                } else {
                    Toast.makeText(this.context, "Error updating vehicle", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this.context, "Error updating vehicle: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
